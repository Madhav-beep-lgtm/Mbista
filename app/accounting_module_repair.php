...
function accounting_module_repair_database(): array
{
    ...
    $run('Hospitality daily sales upload + ledger posting (migration 067)', static function (): void {
        ...
        // The settlement side of a day's sales (migration 122).
        //
        // The item-wise sheet says what was sold and carries the credit side --
        // sales by category, plus VAT. This carries the debit side: cash, card,
        // FonePay, or a customer's own ledger for a credit sale, one row per
        // invoice, so a day settled several ways posts several debits against
        // the one set of credits.
        if (!accounting_repair_table_exists('hospitality_sales_invoice_lines')) {
            db()->exec("CREATE TABLE IF NOT EXISTS `hospitality_sales_invoice_lines` (
                `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
                `upload_id` INT UNSIGNED NOT NULL,
                `company_id` INT UNSIGNED NOT NULL,
                `sale_date` DATE NOT NULL,
